...
def test_user_can_edit_article(
    ...
    expect(editor_page.article_title_input()).to_have_value(existing_article.title)

    expect(editor_page.article_description_input()).to_have_value(existing_article.description)

    expect(editor_page.article_body_input()).to_have_value(existing_article.body)

    # Exercise the behavior under test.
    editor_page.fill_article_form(article_update)

    editor_page.publish()

    # Verify persisted result.
    expect(article_page.title(article_update.title)).to_be_visible()

    expect(article_page.body()).to_contain_text(article_update.body)
